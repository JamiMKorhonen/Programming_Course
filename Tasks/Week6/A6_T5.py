...
# Present average result in 2 digit precision. 
# Utilize the Format Specification Mini-Language.
...
def readValues(PSourceFile):
    ...
    for line in Filehandle:
        i += 1
        numberint = int(line.strip())
        if numberint > greatest:
            greatest = numberint
        numbersum = numbersum + numberint
    avg = numbersum / i
    values = str(i) + ";" + str(numbersum) + ";" + str(greatest) + ";" + "{:.2f}".format(avg)
    Filehandle.close()
    return values


def analyseValues(Pvalues):
    text = "Count;Sum;Greatest;Average" + f"\n{Pvalues}\n"
    return text

def main():
    print("Program starting.")
    SourceFile = input("Insert filename: ")

    values = readValues(SourceFile)

    print("#### Number analysis - START ####")
    print(f"File \"{SourceFile}\" results:")

    text = analyseValues(values)
    print(text)
